...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center">
        <p class="page-indicate">Suit/Purchase</p>
        <div style="margin-top:10px;">
        <button id="filterButton" class="btn main-color">Filter</button>
            <button class="btn main-color"><a href="/purchase_suit" style="color: white; text-decoration:none;">Remove Filters</a></button>
            <button class="btn main-color"><i class="fa-solid fa-plus"></i><a href="/add_purchase_suit" style="color:white"> Add Purchase Suit Record</a></button>
        </div>
    </div>

    <div id="filterContainer" class="conatiner-fluid" style="display: none;">
        <div class="row">
            <form class="custom-form mt-3"  action="{{ route('suitpurchasefilter') }}" method="post">
                @csrf
                <div class="row">
        ...
        <div class="col-md-12">
            @if(session('add'))
                <div id="successAlert" class="alert alert-success">
                    Purchase Suit Record has been added successfully!
                </div>
                <script>
                    setTimeout(function(){
            ...
            <div id="deleteAlert" class="alert alert-success">
                Purchase Suit Record has been deleted!
            </div>
            <script>
                setTimeout(function(){
            ...
            <div id="updateAlert" class="alert alert-success">
                Purchase Suit Record has been Updated!
            </div>
            <script>
                setTimeout(function(){
            ...
            <div class="table-responsive ">
                <table class="table table-hover">
                    ...
                    <tbody>
                        @foreach($suitpurchases as $index => $suitpurchase)
                            <tr>
                                <th scope="row">{{ $suitpurchases->firstItem() + $index }}</th>
                                <td>{{ $suitpurchase->variety }}</td>
                                <td>{{ $suitpurchase->meter }}</td>
                                <td>{{ $suitpurchase->price }}</td>
                                <td>{{ $suitpurchase->pay_price }}</td>
                                <td>{{ $suitpurchase->panding_price }}</td>
                                <td>{{ $suitpurchase->supplier }}</td>

                                <td>
                                    @if($suitpurchase->payment_status == "payed")
                                        <span class="badge bg-success">Payed</span>
                                    @else
                                        <span class="badge bg-danger">pending</span>
                                    @endif
                                </td>
                                <td>{{ $suitpurchase->updated_at->format('d-m-Y') }}</td>
                                <td><i style="cursor: pointer" class="fa-solid fa-print" onclick="printInvoice({{ $suitpurchase->id }})"></i></td>
                                <td>
                                    <form action="{{ route('edit_suitpurchase', ['id' => $suitpurchase->id]) }}" method="get" style="display: inline;">
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('delete_suitpurchase', ['id' => $suitpurchase->id]) }}" method="post" style="display: inline;">
                                        @csrf
                                        @method('DELETE')
            ...
            <div>
                {{ $suitpurchases->links('pagination::bootstrap-5') }}
            </div>
...
<script>
     function printInvoice(id) {
            window.open('/purchasesuitinvoice/' + id, '_blank');
        }
