...
@section('content')
    <div class="container-fluid">
        ...
        <div class="row ">
            <h4 class="mt-3 text-secondary font-weight-bold">Supplier: {{ $name }}</h4>
            <div class="col-md-12 ">
                        ...
                        <tbody>
                            @foreach($transactions as $index=> $transaction)
                                <tr>
                                    ...
                                    <td>{{ number_format($transaction->pay, 0, '.', '') }}</td>
                                    <td>{{ number_format($transaction->pending, 0, '.', '') }}</td>
                                    <td>{{ $transaction->created_at->format('d-m-Y') }}</td>
                                    <td>{{ $transaction->addby }}</td>
                                    
                                    <td>
                                        <form action="{{ route('deletesupplierdetail', ['id' => $transaction->id]) }}" method="post" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
